...
def register_release(endpoint: str, token: str, release: Release) -> int:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "extra-bar-updates-release-sync/1",
        },
    )
    try:
...
def main() -> int:
    ...
    try:
        ...
        # Oldest first makes a first-time backfill easy to follow. Keyper is
        # idempotent per track/version, so replaying the full retained feed is safe.
